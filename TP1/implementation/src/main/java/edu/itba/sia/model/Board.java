...
public class Board {
	...
	public static boolean validateBoard(Board b) {
		Board newBoard = new Board(b.size);
		for (int i = 0; i < b.size; i++)
			for (int j = 0; j < b.size; j++) {
				if (!testInsert(b.getTiles()[i][j], i, j, newBoard))
					return false;
			}
		return true;
	}

	public static boolean testInsert(Tile t, int row, int col, Board b) {
		if (b.getTiles()[row][col] != null) {
			return false;
		}
		boolean valid = true;

		if (col + 1 < b.getSize()) {
			valid = t.matches(b.getTiles()[row][col + 1], Direction.WEST);
		} else if (col - 1 >= 0) {
			valid = valid
					&& t.matches(b.getTiles()[row][col - 1], Direction.EAST);
		}
		if (row - 1 >= 0) {
			valid = valid
					&& t.matches(b.getTiles()[row - 1][col], Direction.NORTH);
		}
		if (row + 1 < b.getSize()) {
			valid = valid
					&& t.matches(b.getTiles()[row + 1][col], Direction.SOUTH);
		}
		b.getTiles()[row][col] = t;
	...
	public String toString() {
		...
		for (int i = 0; i < tiles.length; i++) {
			for (int j = 0; j < tiles.length; j++) {
				String tile = tiles[i][j] == null ? "       " : tiles[i][j]
						.toString();
				boardBuilder.append("[");
